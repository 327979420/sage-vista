...
export default function FavoritePatternPage(){
 ...
 return <TrackerShell active="我最喜欢形态" title="我最喜欢形态" subtitle="只在当天精确MACD金叉候选里，检查你真正愿意做的四项形态。">
  <div className="favoritePage">
   <section className="favoriteIntro"><div><small>MY FAVORITE DAILY SETUP · SHARED MACD GATE</small><h2>同一批金叉候选，只看四项形态</h2><p>先复用当天精确日线MACD金叉门票，再检查回调、双底、三推收盘突破和Golden Pocket或EMA20/50/200承接。四项齐全后再看独立风险闸门。4/4不是胜率。</p><p className="favoriteReferenceLine">复杂证据去多因子页看 · 这里专心看形态 · 不重复扫描全市场</p></div><mark>V3.1 · 生产权重 0</mark></section>
   ...
   {data?<>
    <section className="favoriteMetrics"><article><small>数据日</small><b>{data.as_of}</b><span>{data.pattern_version}</span></article><article><small>MACD门票池</small><b>{data.gate?.source_candidate_count??"—"}</b><span>按当日成交额优先</span></article><article><small>形态完整</small><b>{data.summary.entry_ready}</b><span>4/4＋风险清除</span></article><article><small>3/4接近机会</small><b>{data.summary.near_match??nearRows.filter(row=>row.mechanism_profile?.status!=="blocked_near_match").length}</b><span>明确显示缺哪一项</span></article><article><small>风险阻断</small><b>{data.summary.risk_blocked??0}</b><span>形态不能覆盖空头风险</span></article><article><small>实际深查</small><b>{data.gate?.deep_checked_count??"—"}</b><span>与门票池完全一致</span></article></section>
