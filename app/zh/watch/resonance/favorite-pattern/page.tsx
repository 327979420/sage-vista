...
export const turnover = (n:number, locale: "zh"|"en" = "zh")=>locale==="en"?`$${(n/(n>=1e9?1e9:1e6)).toLocaleString("en-US",{maximumFractionDigits:1})}${n>=1e9?"B":"M"}`:n>=1e8?`${(n/1e8).toFixed(1)} 亿美元`:`${(n/1e4).toFixed(0)} 万美元`;
...
function ShapeChart({row}:{row:Shape}) {
  ...
  return <Localized><div className="shapeChartFrame"><div className="shapeChartScroll"><svg viewBox={`0 0 ${width} ${height}`} role="img" aria-label={`${row.symbol} 日线形态：底部确认与三推标注`}>
  ...
  </svg></div><p className="shapeLegend">绿色区域：支撑区 · 紫线：三推下降线 · 蓝点：已确认转折 · 棕三角：底部确认日</p></div></Localized>;
}
...
export default function FavoritePatternPage() {
  ...
  return <DailyPatternView data={data} error={error} onRetry={()=>{setError("");setData(null);setAttempt(n=>n+1)}}/>;
}

export function DailyPatternView({data,error="",onRetry}: {data:Picker|null;error?:string;onRetry:()=>void}) {
      ...
      {error?<div className="shapeMessage" role="alert"><p>{error}</p><button type="button" onClick={onRetry}>重新加载</button></div>:!data?<div className="shapeMessage" role="status">正在读取日线形态…</div>:<>
        ...
        {selected?<div className="shapeWorkspace"><aside aria-label="形态候选股票">{rows.map(row=><button key={row.symbol} type="button" aria-pressed={selected.symbol===row.symbol} onClick={()=>setSymbol(row.symbol)}><span><b>{row.symbol}</b><small>{row.three_push?"三推回调＋多底":"多底支撑"}</small></span><span><b>{money(row.price)}</b><small>热度 #{row.activity_rank}</small></span></button>)}</aside>
          <article className="shapeDetail" key={selected.symbol}><header><div><small>日线形态 · {selected.as_of}</small><h2>{selected.symbol} · {selected.shape_zh}</h2></div><span className="shapeBadge">{selected.state_zh}</span></header>
            <p>{selected.explanation_zh}</p><p className="shapeScope">当日成交额 {turnover(selected.liquidity.dollar_volume,locale)}{selected.liquidity.relative_volume!=null?` · 成交量为此前20日均量的 ${selected.liquidity.relative_volume.toFixed(2)} 倍`:""}</p>
