...
function GateTags({row}:{row:Row}){
 ...
 return <span className="candidateGateTags">{items.map(p=><span className={`candidateGateTag gate-${p.path}`} key={`${p.path}-${p.timeframe}`}>{pathNames[p.path]??p.path} · {frameNames[p.timeframe]}</span>)}</span>;
}
