...
function PeriodFactors({data,symbol}:{data:CandidateData;symbol:string}){
 ...
 return <div className="v2Ledger periodFactorLedger">{momentum&&<section className="momentumSummary"><h4>突破回踩动能检查</h4><p>三根负柱：{momentum.momentum.histogram.map(v=>v.toFixed(4)).join(" → ")}<small>{momentum.momentum.dates.join(" / ")}</small></p><p>{momentum.confirmed?"满足突破回踩动能门票":explain(momentum.reason)}</p><p>大实体双短影阴线：{!momentum.bearish_candle.available?"证据不足":momentum.bearish_candle.blocked?"有，阻止本次动能门票":"未发现"}</p><small>满足门票后仍需通过方向与位置检查，不代表已执行买入。</small></section>}{detail.checks&&<section><h4>本次准入检查（含排除原因）</h4>{Object.entries(detail.checks).map(([key,c])=><p className="periodFactorPair" key={key}><span>{explain(c.reason)}</span><b>{c.status==="allowed"?"通过":c.status==="blocked"?"未通过":"证据不足"}</b></p>)}</section>}<details><summary>核对门票原始结构底部</summary><small>这里是旧门票保存的结构底，不是当前回踩支撑或已确定的交易止损。当前回踩支撑规则正在核对。</small>{detail.structures?.filter(p=>p.state==="active").map((p,i)=><p className="periodFactorPair" key={i}><span>{frameNames[p.timeframe]} · {pathNames[p.path]}<small>触发 {p.trigger_date}</small></span><b>${p.structure_floor.toFixed(2)}</b></p>)}</details><label className="factorVisibility"><input type="checkbox" checked={showAll} onChange={e=>setShowAll(e.target.checked)}/> 显示全部检查（含未命中与缺数据）</label>{Object.entries(frameNames).map(([tf,name])=>{
