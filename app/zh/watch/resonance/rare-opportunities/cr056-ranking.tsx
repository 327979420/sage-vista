...
type WatchEntry={path:string;timeframe:string;trigger_date:string;trigger_close:number;state:string;invalidated_at:string|null;observation_return:number;observed_sessions:number};
type Row={watch_entries?:WatchEntry[];watch_since?:string|null;watch_return?:number|null;watch_as_of?:string;entry_paths?:{path:string;timeframe:string;confirmed_through:string;cross_date:string|null}[];symbol:string;rank:number|null;price:number|null;status:string;new_nomination:boolean;origin_date:string|null;total:number|null;coverage:number|null;frames:Record<string,number>|null;periods:{monthly:string;weekly:string}|null;high_score_eligible:boolean;reason_codes:string[];checks?:Record<string,{status:string;reason:string}>;groups?:Group[]};
...
function GateTags({row}:{row:Row}){
 ...
 return <span className="candidateGateTags">{kinds.map(kind=><span className={`candidateGateTag gate-${kind}`} key={kind}>{pathNames[kind]??kind} · {["monthly_completed","weekly_completed","daily"].filter(tf=>items.some(p=>p.path===kind&&p.timeframe===tf)).map(tf=>frameNames[tf].replace("线","")).join("/")}</span>)}</span>;
}
 ...
 return <div className="candidateSurface">
  ...
  <section className="researchReplay candidateWorkspace">
   ...
   {selected&&<article className="v2Audit candidateDetail"><header><div><small>{data.as_of} · {statusName(selected)}</small><h3>{selected.symbol} · 分项与原因</h3><p>月线截止 {selected.periods?.monthly??"不可用"} · 周线截止 {selected.periods?.weekly??"不可用"}</p></div><strong>{selected.total?.toFixed(2)??"未入榜"}<small>{selected.total===null?"分项仅供诊断":"人工复核优先级"}</small></strong></header>
    ...
    {!!selected.watch_entries?.length&&<div className="candidateWatchReturns"><p>各类最近门票 · 观察涨跌，不是实际交易收益</p>{selected.watch_entries.map(p=><p key={`${p.path}-${p.timeframe}`}><span>{frameNames[p.timeframe]} · {pathNames[p.path]}<small>{p.trigger_date} 收盘 ${p.trigger_close.toFixed(2)} · {p.observed_sessions}个交易日{p.state!=="active"?` · 结构失效 ${p.invalidated_at}`:" · 结构仍有效"}</small></span><b>{returnText(p.observation_return)}</b></p>)}<small>截至 {selected.watch_as_of}，不含交易费用和分红。表格取最早仍有效门票；原提名日期单独保留。历史重核不是当时已发出的真实警报。</small></div>}
