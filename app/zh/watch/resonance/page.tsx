...
type SignalHistory={cases:{lifecycle:string;latest_current_status:string}[]};
...
export default function Overview(){
 ...
 const [industry,setIndustry]=useState<IndustryReport|null>(null),[snapshot,setSnapshot]=useState<FactorSnapshot|null>(null),[registry,setRegistry]=useState<FactorRegistry|null>(null),[market,setMarket]=useState<MarketReport|null>(null),[history,setHistory]=useState<SignalHistory|null>(null);
 useEffect(()=>{Promise.all([json("/industry-radar.json"),json("/daily-factor-snapshot.json"),json("/factor-registry.json"),json("/market-etf-watch.json"),json("/signal-history.json")]).then(([i,s,r,m,h])=>{setIndustry(i);setSnapshot(s);setRegistry(r);setMarket(m);setHistory(h)})},[]);
 ...
 return <TrackerShell active="今日研究总览" title="今日研究总览" subtitle="市场 → 行业 → 技术机会 → 多因子证据 → 数据审计。">
  {tracker&&<div className="dailyOverview">
   ...
   <section className="dailyPanel forwardSummary"><header><div><small>FORWARD OBSERVATION</small><h2>真实信号观察</h2><p>生产提醒进入独立账本，尚未成熟的结果保持 Pending。</p></div><a href="/zh/watch/resonance/research?tab=forward">查看 Forward Testing →</a></header><div><b>{history?.cases.filter(x=>x.latest_current_status==="current").length??"—"}</b><span>active forward cases</span><i/>
   <b>{history?.cases.filter(x=>x.lifecycle==="MATURED").length??"—"}</b><span>matured</span></div></section>
