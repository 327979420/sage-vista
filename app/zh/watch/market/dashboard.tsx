...
type Report={schema_version:string;as_of:string;universe:{name:string;observed_at:string;members:string[];common_list_count:number;limitation:string};config:{version:string;indicators:Record<string,Spec>;subscores:Record<string,{label:string;weight:number}>;temperature_bands:{maximum:number;label:string;tone:string}[]};history:Snapshot[]};
...
function Card({id,spec,item,history,latest}:{id:string;spec:Spec;item:Indicator;history:Snapshot[];latest:Snapshot}){
 ...
 return <article className="marketSignal" data-tone={item.reliable?item.level.tone:'neutral'}>
  ...
  <div className="marketSignalBottom"><small>1日 {delta(item.change_1d,changeUnit)}<br/>{item.percentile===null?'历史分位：样本不足':`历史分位 ${num(item.percentile,0)}%`}</small><Sparkline values={history.slice(-21).map(s=>s.indicators[id].reliable?s.indicators[id].value:null)} label={`${spec.label}最近21个交易日趋势`}/></div>
  ...
  <div className="marketDashboard">
   {error?<div className="marketError" role="alert"><h2>大盘数据暂不可用</h2><p>{error}</p><button onClick={()=>{setError('');setAttempt(n=>n+1)}}>重新读取</button></div>:!report||!latest||!temp?<p role="status" className="marketLoading">正在读取大盘日终快照…</p>:<>
    <section className="marketTemperature" data-tone={temp.status.tone} aria-label="Market Temperature">
     <div className="marketTemperatureMain"><p className="marketEyebrow">MARKET TEMPERATURE <span>市场风险温度</span></p><div className="marketScore"><strong>{num(temp.score,0)}</strong><span>/ 100</span><b>{temp.status.label}</b></div><h2>{latest.explanation}</h2><p className="marketScoreNote">分数越高，过热或内部恶化的压力越大。不是涨跌预测。</p>
     ...
     <aside className="marketDimensions"><h3>压力来自哪里</h3>{Object.entries(temp.subscores).map(([id,s])=><div key={id}><span>{report.config.subscores[id].label}<small>{num(s.weight*100,0)}%</small></span><b>{num(s.score,0)}</b><meter min="0" max="100" value={s.score??0} aria-label={`${report.config.subscores[id].label} ${num(s.score,0)}`}/></div>)}</aside>
    </section>
    ...
    {GROUPS.map(([group,label])=><section className="marketGroup" key={group}><header><h2>{label}</h2><small>{group}</small></header><div className="marketCards">{Object.entries(report.config.indicators).filter(([,spec])=>spec.group===group).map(([id,spec])=><Card key={id} id={id} spec={spec} item={latest.indicators[id]} history={report.history} latest={latest}/>)}</div></section>)}
