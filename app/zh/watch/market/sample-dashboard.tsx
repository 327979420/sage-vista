...
export function TrendChart({title,description,dates,lines,fixedRange}:{title:string;description:string;dates:string[];lines:Line[];fixedRange?:[number,number]}){
...
export default function SampleDashboard(){
 ...
 return <div className="marketSampleContent">
  <div className="marketDashboard">
   {error?<div className="marketError" role="alert"><h2>大盘数据暂不可用</h2><p>{error}</p><button onClick={()=>{setError('');setAttempt(n=>n+1)}}>重新读取</button></div>:!report||!latest||!temp?<p role="status" className="marketLoading">正在读取大盘日终快照…</p>:<>
    <section className="marketTemperature" data-tone="neutral" aria-label="实验性样本温度">
     <div className="marketTemperatureMain"><p className="marketEyebrow">MARKET TEMPERATURE <span>实验性样本温度</span></p><div className="marketScore"><strong>{num(temp.score,1)}</strong><span>/ 100</span><b>实验性 · 未验证</b></div><h2>{latest.explanation}</h2><p className="marketScoreNote">分数越高，综合压力越大。仅描述这批样本；未验证预测能力，不表示市场安全。</p>
    ...
    <div className="marketSignalsHeading"><h2>样本参与信号</h2><p><span className="marketColorKey" data-tone="green"/>正常 / 健康 <span className="marketColorKey" data-tone="yellow"/>留意 <span className="marketColorKey" data-tone="orange"/>压力升高 <span className="marketColorKey" data-tone="red"/>极端压力</p></div>
    ...
    <section className="marketHistory"><header className="marketHistoryHeading"><div><p className="marketEyebrow">THE BIGGER PICTURE</p><h2>把今天放回趋势里</h2></div><div className="marketPeriods" role="group" aria-label="历史区间">{PERIODS.map(([label,n])=><button key={label} aria-pressed={period===n} onClick={()=>setPeriod(n)}>{label}</button>)}</div></header>
     ...
     <TrendChart title="实验性样本温度" description="分数上升表示压力增加；历史不足时不生成温度。" dates={dates} fixedRange={[0,100]} lines={[{label:'风险温度',color:'#b5682f',values:history.map(s=>s.temperature.score)}]}/>
