...
- 设计状态：`verified`；包A—E及D1/D1b/D1c已完成本地实现、测试和提交，尚未合并、推送或部署
...
- 本地实施状态：包A规则、包B验证器、包C只读适配器、包D影子Manifest、D1/D1b/D1c安全修复及包E本地验收均已完成。实现提交为`10174908879a6217360998b02c781c6e97da67a7`；生产尚未采用Manifest，因此状态只能是`verified`，不能写成`implemented`。
...
M01达到`implemented`只代表：合同定义完成；验证器完成；旧数据只读适配完成；影子Manifest能够生成和验证；测试通过；代码和文档已提交。

它不代表生产已经改用Manifest，不代表网站或Discord已经切换，不代表每日工作流已经改变，也不代表线上已经发布Manifest。这些生产状态必须等待M12独立批准、实施、部署和线上核验。
