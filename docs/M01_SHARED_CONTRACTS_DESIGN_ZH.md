...
所有必需文件必须满足：存在；结构主版本可识别；其业务截止日按合同映射后等于Manifest的`as_of`；防未来明确为`false`；大小和哈希一致；稳定ID在各自作用域内唯一。历史覆盖的开始日可以不同，但生产末日必须一致。
...
以上都是计划，不授权实施。原设计中的每日链、网站、Discord、线上核验、生产部署和回退演练已完整移到“M12未来集成任务”，没有删除。
