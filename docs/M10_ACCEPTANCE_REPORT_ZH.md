...
| pending冻结1／5／20／60／100各窗口权威目标交易日；端点日期和价格必须来自该日历对应行 | 通过 |
...
- M08与M10专项：84项通过。
- M01—M10扩大定向（含M02行情仓）：265项通过。
- 完整Python：621项通过。
- `PYTHONHASHSEED=0/1/42/12345`：每轮84项通过。
- 治理状态：19项通过。
- 前端：lint、TypeScript、生产构建及11项测试通过。
- Python编译和差异格式检查：通过。
- 新增3项机械回归进一步证明：pending保存完整权威session日期序列；1／5／20／60／100任一窗口换成错误但合法的日历日期，或端点日期正确但使用相邻日价格，均不能complete；明确的5日端点`2026-09-09`改为`2026-09-08`并重建合法结果身份仍失败关闭。目标日行情缺失不向前或向后替代，pending／partial／unavailable均保留目标日期证据；合法五窗口、每日／回放同源及幂等重放继续通过。原有日历指纹、落盘pending根、结果全集、ExitState及事件证券绑定反例继续拒绝。
- 测试前后没有出现范围外文件。
