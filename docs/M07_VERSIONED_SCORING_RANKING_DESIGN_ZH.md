...
- 状态：`approved`（A—E影子实施已获批准；尚未完成本地验收、合并、部署或生产启用）
- 基线：`8e18263c8ecaf1f29268378881b1739aeea99701`
- 边界：本文件只设计，不授权实施、提交、推送、合并、部署或生产切换
...
首个迁移政策只允许复现当前已批准的技术共振排序语义；不在M07重构中研究新权重。上下文是否影响同分必须写进政策，不能由调用者临时决定。
...
1. 推荐：首个formal影子政策只复现当前技术共振与既有确定性排序，不引入新权重；M06上下文只按政策明确的既有同分边界使用。
