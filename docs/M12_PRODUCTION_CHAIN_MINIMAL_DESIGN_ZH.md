...
成员采集必须在纽约交易日D内完成，记录实际观察日期，不把D+1抓取的当前列表标成D。复用现有EOD触发中D收盘后、纽约午夜前的窗口先保存成员源（现有23:47、次日00:17至03:17 UTC白名单在美东日期仍为D），价格尚未就绪可稍后重试；若这些窗口全部错过且没有D归档，则D formal unavailable，保留旧页面，不用次日名单补造。交易日／收盘时间须由已核验的完整会话来源确认，节假日不开新日。2026-09-07前提勘误：旧入口实际仅用SPY最新bar确定参考日，尚无已核实的完整交易日历；具体来源与缺项见[价格／日历接入细化卡v0.1](M12_PRICE_CALENDAR_INTAKE_REVIEW_ZH.md)（该卡design_review，不代表价格取证已实施）。上线前演练必须确认现有触发窗口能覆盖此采集期，不另造第二条扫描链。
