...
版本：`0.2.1-draft`；日期：2026-09-06；状态：`approved`（实施进度见CR-053）。

关联CR-2026-09-06-053。设计提交505c89f已通过独立复核，用户明确批准按v0.2.1分包实施；不授权合并、推送、部署、生产启用或对外通知。合并`14fef535f67b7c4de035b4c84224e604850f1fed`与治理`be1119e643ea9f5aa9f63dd25223ee6848dda1f5`已由用户确认通过独立审核；M11不重复审核，CR-043保持captured。本版替代0.1草案的未冻结技术选项，上线日期和实际部署授权留在上线卡。
...
成员采集必须在纽约交易日D内完成，记录实际观察日期，不把D+1抓取的当前列表标成D。复用现有EOD触发中D收盘后、纽约午夜前的窗口先保存成员源（现有23:47、次日00:17至03:17 UTC白名单在美东日期仍为D），价格尚未就绪可稍后重试；若这些窗口全部错过且没有D归档，则D formal unavailable，保留旧页面，不用次日名单补造。交易日／收盘时间须由已核验的完整会话来源确认，节假日不开新日。2026-09-07前提勘误：旧入口实际仅用SPY最新bar确定参考日，尚无已核实的完整交易日历；具体来源与缺项见[价格／日历接入细化卡（当前修订待审）](M12_PRICE_CALENDAR_INTAKE_REVIEW_ZH.md)（该卡design_review，不代表价格取证已实施）。上线前演练必须确认现有触发窗口能覆盖此采集期，不另造第二条扫描链。
...
上线卡只填写实际首次生效日期、已获批准的配置与运行提交、云资源实物ID／权限与数据授权证据、真实dry-run覆盖／差异／成本、回退演练及部署／通知批准。技术栈、合同、股票池范围、身份、政策、任务关系与锁／回退方案均已在本稿推荐冻结，不再交用户决定实现细节。若实际数据源或平台不能满足此设计，失败关闭并将差异交回审核，不能现场换口径。本稿已批准分包实施；上线卡仍须单独批准，尚未授权上线。
