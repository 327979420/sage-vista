...
- 状态：`implemented`（A—E已实现、测试、独立复核并纯fast-forward进入`main`；未部署或生产启用）
...
- 规则／设计：`8818fa7a6911879002e0c5f679dda3b396b83aa4`。
- 唯一事件生产器、只追加存储、旧账适配及每日／回放影子入口：`0d1c2526360b0753d0ca4f2fbec4013b8ee5c2fe`。
- 公开合同与反例测试：`7ca8ea37a0342fea3ed5e10887ca8053295d9040`。
- M09专项19项、M01—M09定向194项、完整Python550项、四种固定哈希种子每轮19项、治理19项和前端11项通过；Python编译、lint、TypeScript、生产构建及差异格式检查通过。
- 快速独立审核分别核对事件／关联记录身份、权威与legacy边界、追加存储和范围；审核发现的同Gate同模型重复判断缺口已在唯一M05批次验证入口修复并由有效伪造反例锁定，复核通过。
- 当前真实旧账只读适配得到4,451个Opportunity记录和69个Signal记录；仅明确ID可以匹配，ticker＋日期不能证明同一事件，继续标记`ambiguous`。适配前后两份旧文件字节完全不变。

`implemented`只表示获批影子范围已进入`main`；不表示部署、生产迁移、收益评价、Excel或M10—M12启动。
