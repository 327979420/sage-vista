...
- 检索全部542个Git跟踪路径及页面、测试、生产工作流引用；本轮定点移除候选页整套旧排行榜/旧四象限/旧机会账本/旧支撑计划显示代码，页面仅承载现有新版候选组件。
- 删除 `app/zh/watch/resonance/rare-opportunities/timeframe-profile.tsx`：唯一运行消费者是上述移除的旧界面；同步更新其旧UI断言，删除无剩余组件引用的旧四象限/研究结论专用样式。月周日事实及后台旧历史计算保留，现行三周期分数仍由新版候选组件显示。
- 跟踪文件名未发现Star/Northstar；两个早期EODHD研究脚本中的northstar是固定抽样种子，有历史复现用途，保留。旧行情/信号/实验/许可文件不因旧名字删除。
- 此次是已查明重复界面瘦身，不声称全仓库不存在其他冗余；后续清理须有调用、工作流及实验用途证据。

## 2026-09-07 文件整理

- 合并五份旧产品规格及九份已完成模块验收为 `archive/product-history.md`、`archive/acceptance-history.md`，保留原文与章节链接。旧文件从当前目录删除，Git 历史可恢复。
- 删除无代码、工作流、测试、文档或实验账本引用的旧回测接手提示和 `scripts/dispatch_historical_years.sh`；后者是一次性批量调用脚本，实际 backfill 工作流保留。
- 七份入口文档合并重复流程，日期和版本只引用机器状态。
- 最大文件是被实际页面、扫描和研究消费的数据，保留；未发现可直接删除的孤立 Python 模块。不能以体积或 legacy 名称代替引用证据。
