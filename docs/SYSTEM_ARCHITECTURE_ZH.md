...
当前提交、线上部署、生产数据日、模型版本和回测断点不在本文件手抄。每次从`docs/CURRENT_STATUS_ZH.md`列出的机器来源和Git状态读取。版本号相同不能证明网站与本地代码相同。

分类说明：下面已经确认的产品边界进入正式架构正文；标为`候选设计`的内容仍等待单独批准；标为`原始需求记录`的用户口述原样保留，不代表已经实施。

需求、实施和生产三种状态统一引用`docs/DECISION_LOG_ZH.md`的“三种状态的唯一说明”，本文件不重复维护状态表。
...
## 目标架构方向（需求方向已确认；CR-035实施仍为`design_review`，生产未上线）
...
### 原始需求记录：用户补充构想（完整保留，尚未全部确认）

以下口述用于保存用户原意。其中的因子数量、排序方式、ETF复用、模拟交易和退出设想不自动成为当前规则；确认项应另行提炼进正式正文，未确认项继续作为候选设计。
