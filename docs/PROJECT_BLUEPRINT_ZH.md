...
自动化升级（2026-08-25）：新增 Early Watch／Confirmed 两级状态。GitHub Actions 负责 EODHD 更新、测试、生产构建、preview 与网站 JSON 提交；私有 Sites 发布成功并核对线上日期后，再触发 GitHub notify job 使用 Repository Secret 发送。任何前置环节失败都不更新网站、不发送 Discord。
