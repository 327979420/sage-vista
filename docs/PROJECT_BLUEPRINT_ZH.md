...
Northstar 要成为一个清楚、可信、能长期积累实验结果的 MACD 技术研究助手。用户的真实交易方式是多周期、多指标、多种价格结构共同验证，因此项目要把这些判断转成可复查的因子，并通过长期历史、独立验证期和前向观察判断哪些因素真正有帮助。
...
实施进度（2026-08-25）：已选择第一版使用 Discord Webhook，并建立可替换模板的同源通知入口。5分或以上雷达机会优先，随后为每日 MACD 看涨／看跌榜；日期、防前视、发布顺序和去重均为失败关闭。Webhook 尚未提供，当前只能预览，不会发送；密钥仅允许写入被 Git 忽略的本地 `.env.local`。
