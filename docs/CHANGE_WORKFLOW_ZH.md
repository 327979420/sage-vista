...
这套原则从M03开始适用，用来缩短范围内的重复审批，不降低数据身份、防未来、formal／legacy隔离、不可变历史、唯一生产者或生产保护要求。

1. 边界严格，内部简洁；当前模块只解决自己的唯一职责，不提前设计后续模块合同。
2. 每个业务事实只有一个计算入口，每个合同只有一个验证入口，不建立备用的第二套生产者。
3. 每日与回测复用同一纯函数，运行模式和`as_of`必须显式传入；消费者只读结果，不复制生产逻辑。
4. 配置集中管理，不散落魔法数字；旧格式只有一个只读适配器。
5. 未决策略标记为`deferred`或`captured`，到责任模块再设计。
6. 只为当前可复现风险建立测试，不为假想未来需求建设复杂框架。
7. 测试优先覆盖公开接口、身份、时间边界、失败关闭和行为等价，避免绑定无意义内部细节。
8. 旧代码只有在消费者、工作流、测试、文档和研究复现引用全部清零后才删除。
9. 发现问题优先修唯一入口和根因，不在消费者旁边堆补丁。
10. 模块满足“唯一职责、输入输出清楚、关键样本通过、无越界、有回退、文档代码一致”后即收口前进，不因仍可继续优化而无限停留。
11. 已批准范围内的安全小包可以连续实施；只有范围扩张、业务语义未决、生产变化、测试无法解释或用户资产冲突时才停止重新评审。

M03—M13设计包默认只需包含：当前重复、一句话职责、输入、输出、唯一负责人、明确不改范围、迁移方式、5—10项关键验收、回退方法和不超过20分钟的实现小包。复杂风险需要更多证据时可以增加，但不能借“完整”提前设计后续模块。
...
- 每次只处理一个CR中的一个可独立验证工作包。若用户已经批准同一CR内连续实施，范围内小包可以连续完成并分别留证，不必逐包重新申请。
