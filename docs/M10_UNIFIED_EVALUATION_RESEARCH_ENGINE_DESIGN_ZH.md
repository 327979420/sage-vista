...
现有`ExperimentRun 2.0.0`已有`parent_run_id`和`checkpoint_ref`，但`checkpoint_ref`只有ID与指纹，`EvaluationShadowStore`也没有可验证的检查点payload；它不能独立证明完成／待处理分区。因此实施所需的最小合同变化是增加一个纯编排证据`ResearchRunCheckpoint 2.0.0`及唯一验证／只追加存储入口，不修改ExperimentRun或任何结果语义。检查点只保存：自身ID／内容指纹、config引用、父run、规范有序的work unit全集、已完成／待处理分区、已保存结果ID／指纹集合、数据／股票池／政策／代码提交证据和生成时间；不保存行情、不计算结果。检查点只允许非终态`in_progress`或`ready_to_finalize`；`completed/failed/interrupted`只能由ExperimentRun收据表达。
...
- 独立审核复现的四项阻断已经由`a0ab77c332995bb2710faa9d3ee946285c1cf0d1`窄修复：M10-C编排只把bundle视为声明并从配置指定store解析唯一当前、已完成的Outcome；成功、失败、中断、重试和摘要共同使用磁盘持久状态盘点；checkpoint不再表达终态，terminal不可写时诚实保留pending且允许同配置只重试finalize；M10-E整数使用严格`type(value) is int`验证。
- 提交链为设计`2517fa6`、E1配置合同`b22da0a`、E2／E3 CLI、检查点、续跑与并发`dbde7fc61c1dcac0959c838552b23051d114b361`和审核修复`a0ab77c332995bb2710faa9d3ee946285c1cf0d1`。修复后本地验收为M10-E专项38项、M10 A—E相关定向176项（跳过10项）、M01—M10扩大定向361项（跳过10项）、完整Python 728项（跳过10项）、四种固定哈希种子每轮38项、治理19项及前端11项通过；Python编译、lint、TypeScript、生产构建、文档链接和格式检查通过。
