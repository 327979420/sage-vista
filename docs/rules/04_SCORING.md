...
## 机会级别与买卖模型的衔接（2026-09-15）

用户要求先辨认日/周/月机会，再选择对应持有、入场、支撑与退出规则。候选资格、评分排名、机会级别和买入就绪分开；日线择时不自动改变月线机会级别。执行定义及20年复用只维护在[08模块“机会级别先行”](08_BACKTEST_AND_EXPERIMENTS.md#机会级别先行所有后续回测与模拟共用2026-09-15用户明确要求)。本次不改生产评分、权重、门票或排行榜；先研究分类，只有证据显示候选选择必须改变时才另登记变更。下方历史V3贡献占比定义不等于当前3.5原始分差阈值，不能直接迁移。
