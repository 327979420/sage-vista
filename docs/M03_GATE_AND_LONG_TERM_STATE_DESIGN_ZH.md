...
- 设计状态：已批准
- 实施状态：`verified`；本地影子A— I已完成并提交，等待人工合并审核
- 生产状态：未部署、未启用
...
M03已在获批范围完成A— I、固定本地案例边界、完整测试、代码与证据提交，因此达到本地`verified`。真实CGEM／MRNA／BTDR／DLTR全历史行情不在本地，验收没有伪造它们的OHLCV，也没有把合成固定样本当成收益证明。若生产尚未切换，必须继续写明“未部署、未启用”；不得把进入`main`自动写成生产采用。

当前结论：**M03为本地`verified`，尚未推送、合并、部署或生产启用。M04与M12均未开始。**
