...
- 本文性质：正式设计版与人话版合一；它不是实施许可，也不改变当前选股结果
...
→ 到达完整GateEvent创建边界
→ 保存当前长期趋势基线等价结果
→ 影子保存局部0.618／70%、多年回撤、完整月线长期状态和持续供给
→ 下游只引用唯一GateEvent
```

这里有两条不能混淆的线：前两步决定“有没有完整事件”；现有长期趋势基线只决定`baseline_passed`；其余新增结构和长期事实只作影子观察。任何影子判断都不能倒过来删除事件、改变`baseline_passed`或改变当前生产结果。
...
当前没有唯一门卫，也没有完整M03 `GateEvent`。最接近主流程的是：
...
推荐把唯一门卫放在中立目录`services/gates/`，而不是放进扫描器、因子模型或回测器。建议职责如下：

| 未来文件 | 唯一职责 |
|---|---|
| `services/gates/policy.py` | 冻结门卫顺序、版本和资格语义 |
| `services/gates/macd.py` | 精确日线MACD刚金叉；只使用完成日线 |
...
| `services/gates/legacy_adapter.py` | 只读解释旧门票，不补造缺失事实、不回写历史 |
...
- `shadow_assessment`：保存新增结构和长期研究事实，必须带`production_effect=false`。它可以给出`shadow_exclusion_candidate`等观察建议，但不能改变`baseline_passed`、排名、每日输出或回测入选。
...
| `unified_v2_scan._candidate()`对上述长期资格的要求 | 当前复杂多因子基线消费语义 | 影子期逐候选对照，不改变现有排名输入 |
| 稀有机会的60日回调／长期趋势、Tracker高周期组合 | legacy消费者私有语义 | 默认结果保持不变，不能冒充M03共同基线 |
| `long_base_reversal`、`broad_range`、`structural_damage`、多年回撤、0.618／70%和持续供给 | 新增影子研究 | 结构化保存，生产权重为零，不改变资格或排名 |
...
```json
{
  ...
  "shadow_assessment": {
    ...
    "feature_schema_version": "m03-gate-features-1.0.0",
...
该结构方向已经写入总需求，但当前生产尚未执行。M03首轮只能作为`shadow_assessment.local_structure`计算并与旧结果并列；`structure_broken`也只能形成影子排除建议，不能把`baseline_passed`改成`false`。激活前必须由用户单独审核反例、受影响事件和回退证据，并升级相应门卫、评分或发布政策。

### 7.4 多年回撤事实

保存点时历史窗口、确认高低点、最大回撤、发生日期、是否已经有足够后续底部消化。它回答“曾经跌多深”，不单独回答“现在能买吗”。

没有实验批准时，多年深跌不得直接改变生产资格、分数或排行；MRNA只用于验证事实是否被正确看见。
...
DLTR式持续供给不塞进五态枚举。它作为独立事实保存：每次向下跳空日期、区间、回补日／未回补、回补失败、新缺口及最近关键缺口控制权。没有匹配对照实验前，它不能扣分、减仓或硬否决。
...
| 模型资格 | 复杂多因子或个人形态是否继续分析 | 未来M04／M05只引用GateEvent |
| 排名与交易 | 分数、名次、精选、计划、持仓和退出 | M03禁止修改 |
| 生产采用 | 每日、夜间、网站和Discord实际读取新事件 | 属于以后受控迁移与M12，不由本文授权 |

### 8.1 评分可进化，但GateEvent不能被评分绑死

M03只提供门卫结论和可复用事实，不设计最终评分公式。GateEvent不得保存一个无法解释来源的综合总分；数据完整性、流动性、MACD、长期状态、回撤、箱体、pivot、缺口和持续供给都应保留结构化字段，而不是只留下加减分结果。

以下五种版本必须分开：

| 版本 | 唯一职责 | 是否参与GateEvent身份 |
|---|---|---|
| `gate_policy_version` | 决定事件创建边界和`baseline_passed` | 是 |
| `feature_schema_version` | 定义可以提供给评分的事实字段及计算语义 | 作为事件内容证据；公式变化产生新特征版本或输入指纹 |
| `score_policy_version` | 定义评分公式、权重、分段、封顶和缺失值处理 | 否；属于下游不可变派生结果 |
| `ranking_policy_version` | 定义同分处理、排序和候选截断 | 否；排名必须明确引用评分版本 |
| `publication_policy_version` | 定义哪些结果可以进入生产输出 | 否；生产启用需独立上线审核 |

长期筑底、多年深跌、宽幅箱体、持续供给和0.618／70%在M03先作为特征保存；在评分政策单独获批前，生产权重必须为零。事实缺失必须表示为`unavailable`，不能静默当成零分。

### 8.2 未来ScoreResult边界

未来每次评分生成独立、不可变的`ScoreResult`或等价派生记录，至少绑定：
...
`score_result_id`至少对`gate_event_id + feature_schema_version + score_policy_version + 规范化评分输入指纹`做规范哈希；审计生成时间不参与身份。每个评分政策必须逐项定义`unavailable`怎样处理，不能依赖调用者默认补零。同一GateEvent与同一评分政策重复计算必须得到相同身份和结果；新评分政策产生新ScoreResult，旧结果永久保留。修改评分公式或权重不得修改原始GateEvent，也不得原地覆盖旧评分。排名必须消费显式指定版本的ScoreResult，禁止默认取“当前最新”破坏历史复现。同一历史日期可以并列保存多个影子评分版本，但不得悄悄替换当时生产结果。

评分变化不得自动改变GateEvent是否存在、`baseline_passed`、股票池身份、市场快照身份或已经发布的正式结果。未来变化分级如下：

- 只改权重或阈值：新增`score_policy_version`并重算派生评分；
- 改特征计算公式：升级`feature_schema_version`，重生成受影响特征和评分；
- 改门卫资格：升级`gate_policy_version`并单独审核，不能伪装成评分调整；
- 改生产发布条件：升级`publication_policy_version`并独立上线审核。

未来实现只能由一个版本化评分政策注册表同时服务每日与回测，不得把阈值、权重或特征开关散落在扫描器、个人形态、回测器和网站。配置可调整不等于可以无版本覆盖：任何影响结果的配置变化都必须形成新版本或新内容指纹并保留旧配置。本轮不定义权重、不实现ScoreResult；评分政策的设计、校准、验证和生产启用属于后续独立CR。

## 九、两个分析器怎样共享同一GateEvent

目标状态：

- 复杂多因子只能接收唯一`gate_event_id`，不得再调用`long_trend_ok()`或自算MACD门票；
- 个人形态只能引用同一股票同一天的GateEvent事实，不得创建第二个门卫ID；
- 两者的分析结果分别保存为独立`ModelAssessment`；复杂多因子仍是唯一主榜，个人形态仍是独立观察，不建立竞争主榜；
- 同一股票同一天最终只建立一个`OpportunityEvent`，其中可以关联两个独立评估。
...
| 复杂多因子 | 未来不再自算长期资格和MACD | 分数、排名不变 | M04只消费GateEvent |
| 个人形态 | 未来引用同一事件身份 | 观察结果不变 | 兼容差异留M05批准 |
| 评分与排行 | 未来消费版本化事实和独立ScoreResult | 公式、权重、分数和名次不变 | 后续独立CR设计评分政策与注册表 |
| 事件账 | 未来新事件保存`gate_event_id` | 不重写历史 | 只追加引用，保持机器证据不可覆盖 |
| 网站／Discord | 将来只展示紧凑门卫解释 | 无变化 | 生产同源发布属于M12 |
| 止损／持仓／退出 | 无职责 | 无变化 | 留给对应后续模块 |

## 十三、四个人工案例的检测走查

这些走查只验证“机器是否看见并如实解释”，不证明收益、不调分、不建立生产排除规则。精确数字必须在未来用统一复权、点时行情重新核验；当前只保存已有案例账事实。
...
人工检查重点：04-17早期机器事件与04-22／24结构确认必须分开。即使影子检测为`long_base_reversal`，也只是假设候选，不改变`baseline_passed`，不自动放行或加分。
...
→ 不使用2024-04-30才完整的月线，也不因后来20日上涨约52%倒改判断
```
...
人工检查重点：箱体内部反弹不能被解释为趋势突破；但在独立实验前也不能直接改变生产排名。
...
→ 不把多个相关缺口机械重复扣分，也不把一个布尔值当完整故事
```

人工检查重点：案例账中的具体日期先逐项核验；持续供给在匹配对照前不扣分、不减仓、不否决。
...
| M03-S01 | 同一GateEvent用两个不同`score_policy_version`评分 | 两个不可变ScoreResult同时保留，互不覆盖 |
| M03-S02 | 只修改评分权重 | GateEvent身份不变；产生新评分政策和新ScoreResult |
| M03-S03 | 修改特征计算公式 | `feature_schema_version`或评分输入指纹必须变化，不能冒充纯权重调整 |
| M03-S04 | 长期状态缺失 | 明确记为`unavailable`并按评分政策处理，不得静默计零 |
| M03-S05 | 运行影子评分 | 当前生产排名、候选截断和发布结果不变 |
| M03-S06 | 历史回放未显式指定评分版本 | 失败关闭，禁止自动选择最新版本 |
| M03-S07 | 每日与回测使用同一事件、特征和评分政策 | 分项得分、总分和ScoreResult身份完全相同 |
| M03-S08 | 比较评分政策变更前后结果 | 新旧结果可并列审计，旧结果和当时生产引用不被覆盖 |
...
8. 核对长期状态、供给事实没有进入正式分数、排行、计划或持仓。
9. 人工并列查看同一GateEvent的两个影子ScoreResult设计样本，确认旧评分不覆盖且事件身份不变；本项只验接口，不实现评分。
...
- ScoreResult、排名政策和发布政策是下游独立版本，不得塞进GateEvent合同版本；
...
8. **包H｜消费者与四案例验收**：验证复杂多因子／个人形态不复制门卫，完成人工案例检测说明；只冻结评分接口边界，不实现ScoreResult、不调分。
...
1. **唯一模块位置**：推荐`services/gates/`，作为M01合同和M02数据之上的中立层；不放进扫描器或回测器。
2. **个人形态兼容**：推荐M03不改当前个人形态；到M05再明确选择“只分析`baseline_passed=true`的共同门票”或“允许观察已到MACD边界但`baseline_passed=false`的同一事件”。没有精确MACD的标的不创建逐股事件，两种方式都禁止第二套门卫。

事件创建边界、影子`production_effect=false`、行情修订链和M02复权复用已作为本次设计修订的合同结论，不再列作开放选择。具体评分公式、权重和生产启用不在M03选择，而必须另立CR。

## 十九、完成定义

M03只有在后续获得实施批准、A— I完成、测试和人工案例通过、代码与证据提交后，才可达到本地`verified`。若生产尚未切换，必须继续写明“未部署、未启用”；不得把进入`main`自动写成生产采用。

当前结论：**M03仍为`design_review`，尚未实施。设计修订完成不等于实施、验证、部署或生产启用。**
