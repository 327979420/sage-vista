...
from services.contracts.validation import publication_preparation_input, _m12_json
...
def execute_preparation_validation(input_bytes: bytes) -> bytes:
    ...
    try:
        value = publication_preparation_input(input_bytes)
        ...
        with PreparationValidationProcess(input_bytes) as child:
            while True:
                ...
                if output is not None:
                    ...
                    if result.get('protocol') != 'm12-preparation-validation/1' or result.get('input_sha256') != 'sha256:' + hashlib.sha256(input_bytes).hexdigest() or type(result.get('input_size_bytes')) is not int or result['input_size_bytes'] != len(input_bytes):
        ...
        raise PreparationExecutionError('preparation computation failed') from None
