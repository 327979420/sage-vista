"""Popular-stock daily shape calibration, reusing CR056 facts; no orders/network.

This is a read-only picker preview. Its output is not a main ranking, a formal
ModelAssessment, a historical alert, or a trading-account input.
"""
...
def build_preview(cache_dir, common_path, *, as_of, top=100, index_path=None):
    ...
    for rank, (symbol, heat, path, digest) in enumerate(selected, 1):
        item = compare_symbol(symbol, json.loads(path.read_bytes()), as_of=as_of,
                              source_sha256=digest, heat=heat)
        item['activity_rank'] = rank
        results.append(item)
    reference_results = []
    available = {r[0]: r for r in candidates}
    for symbol in REFERENCE_CASES:
    ...
    for row in results:
        if not row['selected'] and not row['old'].get('bottom'): row.pop('chart')
