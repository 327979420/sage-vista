...
def historical_examples(start="2025-01-01",limit=20):
 ...
 for path in sorted(pathlib.Path("work/eodhd-cache").glob("*.json")):
  ...
  for i in range(260,len(rows)-1):
   if rows[i]["date"]<start or not (line[i]>signal[i] and line[i-1]<=signal[i-1]):continue
   ...
   hits=[name for name in COMPONENTS if flags.get(f"多因子组件＋{name}")]
...
def run(out="public/rare-opportunity-radar.json",as_of=None):
 ...
 for path in sorted(pathlib.Path("work/eodhd-cache").glob("*.json")):
  ...
  if not (line[i]>signal[i] and line[i-1]<=signal[i-1]):continue
  ...
  if score>=5:signals.append({"symbol":symbol,"date":latest,"price":round(current["close"],2),"score":score,"level":"极稀有" if score==6 else "稀有","components":hits,"dollar_volume":round(current["close"]*current["volume"])})
 ...
 report={"generated_at":datetime.now(timezone.utc).isoformat(),"as_of":latest,"status":"research_observation_only","policy":"5分播报为稀有观察，6分播报为极稀有观察；不是买入信号，不显示为已验证胜率。","scan":{"frequency":"每个美国交易日收盘后","universe_scanned":scanned,"minimum_price":5,"minimum_dollar_volume":10_000_000,"future_data_used":False},"signals":signals,"historical_examples":examples}
