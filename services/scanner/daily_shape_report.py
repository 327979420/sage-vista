...
def chart(row):
    ...
    # Focus the drawing on the current anchors; calculations retain all 121 bars.
    dates = [a['date'] for a in row['bottom']['anchors']] + [a['date'] for a in (row.get('three_push') or {}).get('anchors', [])]
    first = min((i for i,b in enumerate(bars) if b['date'] in dates), default=max(0,len(bars)-50))
    bars = bars[max(0, first-10):]
...
def card(row, teaching=False):
    ...
    group = 'teaching' if teaching else 'waiting' if row['state']=='waiting' else 'tracking' if row['selected'] else 'excluded'
    ...
    old_bottom = old.get('bottom') or {}
    old_dates = ' / '.join(str(old_bottom.get(k) or '—') for k in ('first_date','second_date'))
...
<details><summary>查看底部日期和识别依据</summary><table><thead><tr><th>底部</th><th>发生日</th><th>确认日</th><th>价格</th></tr></thead><tbody>{bottom_details or '<tr><td colspan="4">没有通过共享底部确认</td></tr>'}</tbody></table><p>三推锚点：{escape(anchor_text)}</p><p>旧版双底日期（月/日/年）：{escape(old_dates)}。旧版可使用更长窗口，不等于当前结构仍有效。</p><p>底部确认只说明已有后续K线支持该低点；以后仍可能跌破。已突破标签按当前结构的历史收盘证据核对。</p><small>图与事实窗口：{escape(row['window_fingerprint'])}</small></details></article>'''
...
def write_report(report, out):
    ...
    waiting = [r for r in selected if r['state']=='waiting']
    tracking = [r for r in selected if r['state']!='waiting']
    overview = f"从已覆盖的{report['universe']['eligible_covered_count']}只合格普通股中，核对成交额前{len(rows)}只：{len(waiting)}只底部已确认、等待突破；{len(tracking)}只已越线或进入后续状态；{len(rows)-len(selected)}只未通过。"
    note = '这是固定日期的识别校准，不是今日实时名单。自动检查通过不代表形态准确率已由人工确认；旧版与新版不同，也不自动表示新版正确。'
    body = ''.join(card(r) for r in waiting+tracking+differences)+''.join(card(r, True) for r in report['teaching_cases'])
...
*{box-sizing:border-box}body{margin:0;background:#f3f6f8;color:#172b40;font:16px/1.7 system-ui,sans-serif}main{max-width:1200px;margin:auto;padding:30px 24px}h1{font-size:34px;margin:4px 0}h2{font-size:22px;margin:4px 0}header{display:flex;justify-content:space-between;align-items:center;gap:14px;flex-wrap:wrap}article{padding:25px;background:white;border:1px solid #dce4eb;border-radius:14px;margin:20px 0}.badge{background:#e7eefc;border-radius:7px;padding:6px 12px;font-size:14px}.muted,small{color:#607488}svg{width:100%;height:auto}svg text{font:12px system-ui;fill:#405367}table{width:100%;text-align:left;border-collapse:collapse}td,th{border-bottom:1px solid #e2e8f0;padding:8px}input,select{font:inherit;padding:10px;border:1px solid #bac7d3;border-radius:7px;max-width:100%}.notice{background:#fff5d8;padding:14px;border-radius:9px}details{overflow-wrap:anywhere}summary{cursor:pointer;font-weight:600}@media(max-width:600px){main{padding:16px 10px}article{padding:14px}h1{font-size:27px}}
    ...
    html += f'<small>SAGE VISTA · 识别校准 · {report["as_of"]}</small><h1>热门股 · 日线 · 形态</h1><p>{overview}</p><p>先确认底部，再观察三推回调等结构。没有突破也可以入选。</p><div class="notice">{note}</div><p><label>找股票 <input id="search" placeholder="例如 AAPL" aria-label="按股票代码搜索"></label> <label>查看 <select id="group"><option value="waiting">等待突破</option><option value="tracking">已越线／后续跟踪</option><option value="excluded">旧版有底、新版未通过</option><option value="teaching">教学案例</option><option value="all">全部核图案例</option></select></label></p><p id="count" aria-live="polite"></p>'
    html += body + '<p>全部受检股票状态见同目录“逐股对照.csv”；页面只画入选、分歧和教学案例，避免下载无关股票的历史详情。</p></main>'
    ...
    lines += [f'| {r["symbol"]} | {r["shape_zh"]} | {r["state_zh"]} | {r["old"]["stage_zh"]} |' for r in waiting]
