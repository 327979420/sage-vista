...
def verify_once(base,expected):
 ...
 status=fetch(base,"update-status.json",expected,attempts=1);tracker=fetch(base,"resonance-tracker.json",expected,attempts=1);snapshot=fetch(base,"daily-factor-snapshot.json",expected,attempts=1);radar=fetch(base,"rare-opportunity-radar.json",expected,attempts=1)
 dates={status.get("source_latest_complete_date"),status.get("tracker_as_of"),status.get("factor_snapshot_as_of"),status.get("radar_as_of"),tracker.get("as_of"),snapshot.get("as_of"),radar.get("as_of")}
 ...
 if status.get("future_data_used") is not False or snapshot.get("future_data_used") is not False or radar.get("scan",{}).get("future_data_used") is not False:raise RuntimeError("Live future-data audit failed")
