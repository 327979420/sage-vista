...
from .favorite_pattern_tracker import build_report as build_favorite_pattern_report,evaluate as evaluate_favorite_pattern,should_publish as should_publish_favorite_pattern
...
def run(out="public/resonance-tracker.json",as_of=None):
 ...
 for symbol,new in updates.items():
  ...
  chain_score,chain_reason=transmission_score(frames);macd_buy_valid,macd_gate_reason=macd_buy_gate(frames);macd_sell_valid=macd_sell_gate(frames);base_score=sum(x["macd_score"] for x in frames.values());rsi_score=sum(x["rsi_score"] for x in frames.values());volume=volume_state(adjusted);price_structure=price_structure_state(adjusted);ema_states={name:ema_state(rows) for name,rows in frame_rows.items()};ema_layer=ema_states["日线"];breakout_layer=breakout_state(adjusted);favorite_pattern=evaluate_favorite_pattern(adjusted)
  if should_publish_favorite_pattern(favorite_pattern,symbol):favorite_pattern=evaluate_favorite_pattern(adjusted,include_chart=True)
 ...
 report["favorite_pattern_tracker"]=build_favorite_pattern_report(candidates,latest)
 ...
 published.update(x["symbol"] for x in report["favorite_pattern_tracker"]["candidates"])
