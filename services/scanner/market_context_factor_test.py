...
def run(cache="work/eodhd-panel-v4.json",out="public/market-context-factor-test.json"):
 ...
 for lookback in LOOKBACKS:
  for gate,pair in PAIRS.items():
   a,b=pair.split("/");context={d:ratio_signal(maps[a],maps[b],d,lookback) for d in dates}
   for combo in COMBINATIONS:
    ...
    continuous.append({"gate":gate,"pair":pair,"lookback":lookback,"combination":combo,"splits":splits})
 adjusted=bh_adjust([((x["gate"],x["lookback"],x["combination"]),x["splits"]["development"]["p_one_sided"] or 1) for x in continuous])
 for x in continuous:
