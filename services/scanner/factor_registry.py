...
FACTORS = (
    ...
    factor("qualification.pullback_60d", "距60日高点回调", "close <= prior_60d_high*0.95", "qualification", status="candidate", score_mode="display_only", redundancy="pullback", factor_type="qualification", tier="auxiliary", experimental_weight=1),
    ...
    factor("support.ema_proximity", "日线 EMA21/50/200支撑", "daily close is within registered tolerance of daily EMA21, EMA50 or EMA200", "support", status="candidate", score_mode="observational", weight=1, redundancy="moving_average_support", refs=("macd-multifactor-score-v1-2026-08-25",), factor_type="state", tier="core", experimental_weight=2),
    factor("support.weekly_ema_proximity", "完整周线 EMA20/50/200支撑", "current completed daily close is within -2% to +3% of an EMA20, EMA50 or EMA200 calculated only from completed weekly bars", "support", "weekly_completed", "candidate", "observational", 0, "weekly_moving_average_support", explanation="用已经完整收盘的周K计算 EMA20/50/200；当前价格靠近其中一条周线均线时，记录为更大周期支撑。", factor_type="state", tier="auxiliary", experimental_weight=2),
    factor("support.monthly_ema_proximity", "完整月线 EMA20/50/200支撑", "current completed daily close is within -2% to +5% of an EMA20, EMA50 or EMA200 calculated only from completed monthly bars", "support", "monthly_completed", "candidate", "observational", 0, "monthly_moving_average_support", explanation="用已经完整收盘的月K计算 EMA20/50/200；当前价格靠近其中一条月线均线时，记录为长期结构支撑。", factor_type="state", tier="auxiliary", experimental_weight=3),
    ...
    factor("structure.trendline_three_push", "三推趋势线突破", "confirmed three-push descending trendline close breakout", "price_structure", status="unstable", score_mode="display_only", weight=0, redundancy="trendline_breakout", refs=("macd-pattern-v0.6.0-2026-08-24",), window=10),
    ...
    factor("structure.higher_low", "更高低点", "latest confirmed swing low exceeds prior confirmed swing low", "price_structure", status="pending", redundancy="bottom_structure", delay=2, factor_type="state"),
    ...
    factor("structure.trendline_three_push_retest", "三推突破后回踩确认", "within 10 completed sessions after a confirmed three-push descending-trendline breakout, price touches the projected line within max(2%, 0.5 ATR), does not materially pierce it, and closes on or above it", "price_structure", status="candidate", score_mode="observational", weight=1, redundancy="trendline_breakout", explanation="三推下降趋势线突破后十个交易日内回踩原趋势线并收盘守住，作为突破证据链的附加确认。", depends_on=("structure.trendline_three_push",), window=10, tier="core", experimental_weight=2),
    factor("structure.bullish_fvg_support", "Bullish FVG支撑", "open daily bullish fair-value gap remains below price as support", "price_structure", status="candidate", score_mode="observational", weight=1, redundancy="imbalance_support", refs=("macd-multifactor-score-v1-2026-08-25",), factor_type="state", tier="core", experimental_weight=2),
    factor("risk.overhead_unfilled_gap", "上方未补跳空缺口", "unfilled downside gap remains overhead", "risk", status="candidate", score_mode="observational", weight=1, redundancy="overhead_supply", refs=("macd-multifactor-score-v1-2026-08-25",), factor_type="risk", tier="core", experimental_weight=2),
    ...
    factor("structure.bottom_bullish_engulfing", "底部看涨吞没", "bullish engulfing in lower 30% of trailing range within four bars before cross", "price_structure", status="rejected", redundancy="bottom_candle", refs=("macd-candle-v0.7.0-2026-08-24",), window=5),
    factor("structure.support_bullish_engulfing", "支撑位看涨吞没", "the latest completed candle is bullish, the prior candle is bearish, the bullish real body fully engulfs the prior bearish real body, and at least one registered support context is active", "price_structure", status="candidate", score_mode="observational", weight=1, redundancy="support_candle_confirmation", refs=("support-confirmation-hypothesis-2026-08-25",), explanation="在已登记支撑附近，后一根阳线实体完整包住前一根阴线实体，作为支撑获得价格响应的观察确认。", window=5, tier="core", experimental_weight=2, dependency_policy="support_context"),
    factor("structure.weekly_bullish_engulfing", "完整周线看涨吞没", "the latest completed weekly bullish real body fully engulfs the prior completed weekly bearish real body", "price_structure", "weekly_completed", "candidate", "observational", 0, "weekly_engulfing_reversal", explanation="最近一根完整周K阳线实体包住前一根完整周K阴线实体，记录为周线级别的多头反击；实验权重高于日线吞没。", factor_type="state", tier="auxiliary", experimental_weight=3),
    factor("structure.monthly_bullish_engulfing", "完整月线看涨吞没", "the latest completed monthly bullish real body fully engulfs the prior completed monthly bearish real body", "price_structure", "monthly_completed", "candidate", "observational", 0, "monthly_engulfing_reversal", explanation="最近一根完整月K阳线实体包住前一根完整月K阴线实体，记录为月线级别的多头反击；实验权重高于周线吞没。", factor_type="state", tier="auxiliary", experimental_weight=4),
    factor("structure.weekly_double_bullish_engulfing", "完整周线双看涨吞没", "the latest completed week is a bullish engulfing and a prior bullish engulfing occurred within 26 completed weeks with both lows within 10%", "price_structure", "weekly_completed", "candidate", "observational", 0, "weekly_engulfing_reversal", explanation="26个完整周K内出现两次周线看涨吞没，且两个低点相差不超过10%；作为双底式反弹候选，取代同周期单次吞没的实验权重。", depends_on=("structure.weekly_bullish_engulfing",), factor_type="state", tier="auxiliary", experimental_weight=4),
    factor("structure.monthly_double_bullish_engulfing", "完整月线双看涨吞没", "the latest completed month is a bullish engulfing and a prior bullish engulfing occurred within 12 completed months with both lows within 10%", "price_structure", "monthly_completed", "candidate", "observational", 0, "monthly_engulfing_reversal", explanation="12个完整月K内出现两次月线看涨吞没，且两个低点相差不超过10%；作为长期双底式反弹候选，取代同周期单次吞没的实验权重。", depends_on=("structure.monthly_bullish_engulfing",), factor_type="state", tier="auxiliary", experimental_weight=5),
