...
def validate(authoritative,tracker,radar,snapshot):
 if tracker.get("as_of")!=authoritative or radar.get("as_of")!=authoritative or snapshot.get("as_of")!=authoritative:
...
def run(target=1000,as_of=None):
 ...
 current_tracker=read_json(PUBLIC/"resonance-tracker.json");current_radar=read_json(PUBLIC/"rare-opportunity-radar.json");current_snapshot=read_json(PUBLIC/"daily-factor-snapshot.json")
 if current_tracker.get("as_of")==authoritative and current_radar.get("as_of")==authoritative and current_snapshot.get("as_of")==authoritative:
 ...
 with tempfile.TemporaryDirectory(prefix="daily-update-",dir="work") as folder:
  ...
  tracker=run_tracker(folder/"resonance-tracker.json",authoritative);snapshot=run_factor_snapshot(folder/"daily-factor-snapshot.json",authoritative);radar=run_radar(folder/"rare-opportunity-radar.json",authoritative,snapshot)
  validate(authoritative,tracker,radar,snapshot);now=datetime.now(timezone.utc).isoformat()
  status={"status":"up_to_date","market":"US","provider":"EODHD","source_latest_complete_date":authoritative,"tracker_as_of":tracker["as_of"],"factor_snapshot_as_of":snapshot["as_of"],"radar_as_of":radar["as_of"],"data_dates_match":True,"future_data_used":False,"last_successful_update_at":now,"checks":{"provider_date_exact":True,"tracker_snapshot_radar_same_date":True,"completed_bars_only":True,"production_outputs_published_atomically":True}}
  (folder/"update-status.json").write_text(json.dumps(status,ensure_ascii=False,indent=2))
  for name in ("resonance-tracker.json","daily-factor-snapshot.json","rare-opportunity-radar.json","update-status.json"):os.replace(folder/name,PUBLIC/name)
