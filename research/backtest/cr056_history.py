"""Adapt cached historical MACD tickets to the existing CR056 scorer/account.
...
def ticket_dates(rows, start, end):
    line, signal=macd([r['close'] for r in rows])
    return [r['date'] for i,r in enumerate(rows) if i>=MIN_HISTORY_SESSIONS-1
            and start<=r['date']<=end and r['close']>=MIN_CLOSE
            and r['close']*r['volume']>=MIN_DOLLAR_VOLUME
            and line[i]>signal[i] and line[i-1]<=signal[i-1]]
...
def prepare(request, cache_dir, work, code_commit):
    ...
    for path in sorted(cache.glob('*.json')):
        ...
        try:
            ...
            for d in ticket_dates(rows,start,end):tickets[d].append(symbol)
    ...
    for day in sessions:
        ...
        for rank,r in enumerate(selected,1):
            ...
            selection={'rank':rank,'technical_score':r['score']['total_score'],'model_version':POLICY_VERSION,'execution_policy_version':POLICY,
                       ...
                       'reasons':r['reason_codes'],'support_plan':signal_support_plan(past)}
        ...
        state['days'].append({'date':day,'model_version':POLICY_VERSION,'candidate_count':len(ranking),'ranking':ranking,'macd_tickets':len(tickets[day]),'direction_rejected':rejected,'unavailable':unavailable,'missing_session_symbols':missing_sessions,'source_snapshot':report['snapshot_fingerprint']})
